...
public class Main {
	...
	private double getVolumn(double xlow, double xhigh) {
		...
		return PI * (ansPoly.getValue(xhigh) - ansPoly.getValue(xlow));
	}
	...
	private double getMarking(int markNum){
		double targetNum = markNum * inc;
		double up=xhigh, down=xlow;
		double mark = (up+down)/2;
		double differ = getVolumn(xlow, mark) - targetNum;
		while(abs(differ) > 0.001){
			if(differ > 0){
				up = mark;
			}else{
				down = mark;
			...
			mark = (up+down)/2;
			differ = getVolumn(xlow, mark) - targetNum;
		}
		return mark;
	}
